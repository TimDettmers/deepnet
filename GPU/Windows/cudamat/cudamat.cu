...
__declspec(dllexport) extern const char* get_last_cuda_error() {
    cudaError_t err = cudaGetLastError();

    return cudaGetErrorString( err);
}

__declspec(dllexport) extern int cublas_init() {
    cublasInit();
...
__declspec(dllexport) extern int cublas_shutdown() {
    cublasShutdown();
    if (check_cublas_error())
        return CUBLAS_ERROR;
    else
...
__declspec(dllexport) extern int cuda_set_device(int deviceId) {
...
__declspec(dllexport) extern int init_random(rnd_struct* rnd_state, int seed, char* cudamatpath) {
...
__declspec(dllexport) extern int get_leading_dimension(cudamat* mat) {
    return mat->is_trans ? mat->size[1] : mat->size[0];
}

__declspec(dllexport) extern int get_nonleading_dimension(cudamat* mat) {
    return mat->is_trans ? mat->size[0] : mat->size[1];
}

__declspec(dllexport) extern void set_transpose(cudamat* mat, int is_trans) {
...
__declspec(dllexport) extern void cuda_sync_threads() {
    cudaThreadSynchronize();
}

/* ------------------------------ Allocating/moving data ------------------------------ */

__declspec(dllexport) extern int allocate_device_memory(cudamat* mat) {
...
__declspec(dllexport) extern int copy_to_host(cudamat* mat) {
    int len = mat->size[0]*mat->size[1];
...
__declspec(dllexport) extern int copy_to_device(cudamat* mat) {
    int len = mat->size[0]*mat->size[1];
    int err_code = 0;
...
__declspec(dllexport) extern int copy_on_device(cudamat* mat1, cudamat* mat2) {
...
__declspec(dllexport) extern int get_row_slice(cudamat* source, cudamat* target, unsigned int start, unsigned int end) {
...
__declspec(dllexport) extern int set_row_slice(cudamat* source, cudamat* target, unsigned int start, unsigned int end) {
...
__declspec(dllexport) extern int copy_transpose(cudamat* source, cudamat* target) {
...
__declspec(dllexport) extern int free_device_memory(cudamat* mat) {
...
__declspec(dllexport) extern int reshape(cudamat* mat, unsigned int m, unsigned int n) {
...
__declspec(dllexport) extern int get_slice(cudamat* source, cudamat* target, unsigned int first_col, unsigned int last_col) {
    if (source->is_trans)
...
__declspec(dllexport) extern int get_vector_slice(cudamat* source, cudamat* target, unsigned int first_ind, unsigned int last_ind) {
...
__declspec(dllexport) extern void init_from_array(cudamat* mat, float* data, int m, int n) {
    mat->data_host = data;
    mat->size[0] = m;
    mat->size[1] = n;
    mat->on_device = 0;
...
__declspec(dllexport) extern int init_empty(cudamat* mat, int m, int n) {
    mat->size[0] = m;
    mat->size[1] = n;
    mat->on_device = 1;
    mat->on_host = 0;
...
__declspec(dllexport) extern int fill_with_rand(rnd_struct* rnd_state, cudamat* mat) {
    int len = mat->size[0] * mat->size[1];
...
__declspec(dllexport) extern int fill_with_randn(rnd_struct* rnd_state, cudamat* mat) {
    int len = mat->size[0] * mat->size[1];
...
__declspec(dllexport) extern int add_col_vec(cudamat* mat, cudamat* vec, cudamat* target) {
...
__declspec(dllexport) extern int add_col_mult(cudamat* mat, cudamat* vec, cudamat* target, float mult) {
    unsigned int h = mat->size[0],
                 w = mat->size[1];
...
__declspec(dllexport) extern int add_row_vec(cudamat* mat, cudamat* vec, cudamat* target) {
    unsigned int h = mat->size[0],
                 w = mat->size[1];
...
__declspec(dllexport) extern int mult_by_col_vec(cudamat* mat, cudamat* vec, cudamat* target) {
    unsigned int h = mat->size[0],
                 w = mat->size[1];
...
__declspec(dllexport) extern int mult_by_row_vec(cudamat* mat, cudamat* vec, cudamat* target) {
    unsigned int h = mat->size[0],
                 w = mat->size[1];
...
__declspec(dllexport) extern int less_than(cudamat* mat1, cudamat* mat2, cudamat* target) {
    int len = mat1->size[0]*mat1->size[1];
...
__declspec(dllexport) extern int less_than_scalar(cudamat* mat, float val, cudamat* target) {
    int len = mat->size[0]*mat->size[1];
...
__declspec(dllexport) extern int greater_than(cudamat* mat1, cudamat* mat2, cudamat* target) {
    int len = mat1->size[0]*mat1->size[1];
...
__declspec(dllexport) extern int greater_than_scalar(cudamat* mat, float val, cudamat* target) {
    int len = mat->size[0]*mat->size[1];
...
__declspec(dllexport) extern int max_by_axis(cudamat* mat, cudamat* target, int axis) {
    unsigned int h = mat->size[0],
                 w = mat->size[1];
...
__declspec(dllexport) extern int sign(cudamat* mat, cudamat* target) {
    int len = mat->size[0]*mat->size[1];
...
__declspec(dllexport) extern int apply_sigmoid(cudamat* mat, cudamat* target) {
...
__declspec(dllexport) extern int apply_softmax(cudamat* mat, cudamat* target, unsigned int width) {
    unsigned int len = mat->size[0] * mat->size[1];
    if (!mat->on_device || !target->on_device)
        return ERROR_NOT_ON_DEVICE;
    ...
    kApplySoftmax<<<NUM_VECTOR_OP_BLOCKS,NUM_VECTOR_OP_THREADS_PER_BLOCK>>>(mat->data_device, target->data_device, len, width);
...
__declspec(dllexport) extern int apply_sparse_dot(cudamat* mat, cudamat* target, unsigned int dense_columns, cudamat* data, cudamat* ptr, cudamat* indices, unsigned int sparse_rows, unsigned int sparse_columns) {  
    unsigned int dense_len = (mat->size[0] * mat->size[1]);
    if (!mat->on_device || !target->on_device)
        return ERROR_NOT_ON_DEVICE;	

    kApplySparseDot<<<NUM_VECTOR_OP_BLOCKS,dense_columns>>>(mat->data_device, target->data_device, dense_len, dense_columns, data->data_device, ptr->data_device, indices->data_device, sparse_rows, sparse_columns);
...
__declspec(dllexport) extern int apply_sparse_mul(cudamat* mat, cudamat* target, unsigned int dense_columns, cudamat* data, cudamat* ptr, cudamat* indices) {  
    unsigned int dense_len = (mat->size[0] * mat->size[1]);
    if (!mat->on_device || !target->on_device)
        return ERROR_NOT_ON_DEVICE;	

    kApplySparseMul<<<NUM_VECTOR_OP_BLOCKS,NUM_VECTOR_OP_THREADS_PER_BLOCK>>>(mat->data_device, target->data_device, dense_len, dense_columns, data->data_device, ptr->data_device, indices->data_device);
...
__declspec(dllexport) extern int apply_log(cudamat* mat, cudamat* target) {
    unsigned int len = mat->size[0] * mat->size[1];
...
__declspec(dllexport) extern int apply_exp(cudamat* mat, cudamat* target) {
    unsigned int len = mat->size[0] * mat->size[1];
...
__declspec(dllexport) extern int apply_sqrt(cudamat* mat, cudamat* target) {
    unsigned int len = mat->size[0] * mat->size[1];
...
__declspec(dllexport) extern int apply_pow(cudamat* mat, float pow, cudamat* target) {
    unsigned int len = mat->size[0] * mat->size[1];
...
__declspec(dllexport) extern int reciprocal(cudamat* mat, cudamat* target) {
    unsigned int len = mat->size[0] * mat->size[1];
...
__declspec(dllexport) extern int dot(cudamat* mat1, cudamat* mat2, cudamat* target, float beta, float alpha) {
    if (!mat1->on_device || !mat2->on_device || !target->on_device)
        return ERROR_NOT_ON_DEVICE;
...
__declspec(dllexport) extern float vdot(cudamat* mat1, cudamat* mat2, int* err_code) {
    int len = mat1->size[0]*mat1->size[1];
    float res;
...
__declspec(dllexport) extern int add_mult(cudamat* mat1, cudamat* mat2, float alpha) {
    int len = mat1->size[0]*mat1->size[1];
...
__declspec(dllexport) extern int add_elementwise(cudamat* mat1, cudamat* mat2, cudamat* target) {
    int len = mat1->size[0]*mat1->size[1];
...
__declspec(dllexport) extern int subtract_elementwise(cudamat* mat1, cudamat* mat2, cudamat* target) {
    int len = mat1->size[0]*mat1->size[1];
...
__declspec(dllexport) extern int divide_elementwise(cudamat* mat1, cudamat* mat2, cudamat* target) {
    int len = mat1->size[0]*mat1->size[1];
...
__declspec(dllexport) extern int mult_elementwise(cudamat* mat1, cudamat* mat2, cudamat* target) {
    int len = mat1->size[0]*mat1->size[1];
...
__declspec(dllexport) extern int assign_scalar(cudamat* mat, float alpha) {
    int len = mat->size[0]*mat->size[1];
...
__declspec(dllexport) extern int mult_by_scalar(cudamat* mat, float alpha, cudamat* target) {
    int len = mat->size[0]*mat->size[1];
...
__declspec(dllexport) extern int divide_by_scalar(cudamat* mat, float alpha, cudamat* target) {
    int len = mat->size[0]*mat->size[1];
...
__declspec(dllexport) extern int add_scalar(cudamat* mat, float alpha, cudamat* target) {
    int len = mat->size[0]*mat->size[1];
...
__declspec(dllexport) extern float euclid_norm(cudamat* mat, int* err_code) {
    int len = mat->size[0]*mat->size[1];
