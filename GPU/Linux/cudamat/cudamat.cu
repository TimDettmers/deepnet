...
extern int apply_softmax(cudamat* mat, cudamat* target, unsigned int width) {
    unsigned int len = mat->size[0] * mat->size[1];
    if (!mat->on_device || !target->on_device)
        return ERROR_NOT_ON_DEVICE;
...
extern int apply_sparse_dot(cudamat* mat, cudamat* target, unsigned int dense_columns, cudamat* data, cudamat* ptr, cudamat* indices, unsigned int sparse_rows, unsigned int sparse_columns) {  
...
extern int apply_sparse_mul(cudamat* mat, cudamat* target, unsigned int dense_columns, cudamat* data, cudamat* ptr, cudamat* indices) {  
    unsigned int dense_len = (mat->size[0] * mat->size[1]);
    if (!mat->on_device || !target->on_device)
        return ERROR_NOT_ON_DEVICE;	
