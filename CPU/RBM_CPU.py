...
class RBM:
    ...
    def __init__(self, data, weights, epochs, is_sparse = False, activation = lambda X: 1./(1.0-np.exp(-X)), use_noise = False, logger = None):
        self.u = Util()
    ...
    def positive_phase(self):     
        self.h = self.func(np.dot(self.input_dropped,self.w)+self.bias_h) 
        self.w_updt += np.dot(self.input_dropped.T,self.h)        
        self.bias_h_updt += np.sum(self.h,axis=0)
        self.bias_v_updt += np.sum(self.input_dropped,axis=0)          
        
    def gibbs_updates(self, weight_size): 
        self.h = (self.h > np.random.rand(self.current_batch_size,weight_size))       
        
    def negative_phase(self):    
        self.input_dropped = self.func(np.dot(self.h,self.w.T)+self.bias_v)
        self.h = self.func(np.dot(self.input_dropped,self.w)+self.bias_h)
        self.w_updt -= np.dot(self.input_dropped.T,self.h)
        self.bias_h_updt -= np.sum(self.h,axis=0)
        self.bias_v_updt -= np.sum(self.input_dropped,axis=0)
    ...
    def get_visible_vector(self, batch):
        if len(self.trained_weights) > 0:       
            ...
            for weight_pair in self.trained_weights:            
                visible_vector = self.func(np.dot(visible_vector, weight_pair[0]) + weight_pair[1])
            return visible_vector
        else:
            return batch

    def initialize_weights(self, size):
        self.w = np.array(np.random.randn(self.input,size))*0.1        
        self.bias_h = np.zeros((1,size))
        self.bias_v = np.zeros((1,self.input))
        self.w_updt = np.zeros((self.input, size))
        self.bias_h_updt = np.zeros((1,size))
        self.bias_v_updt = np.zeros((1,self.input))
        self.h = np.zeros((100,size))
        self.input_dropped = np.zeros((100,self.input))
        ...
        for current_epochs, weight_size in zip(self.epochs, self.weights_to_do):               
            ...
            for epoch in xrange(current_epochs):    
                ...
                for start_idx in range(0,self.X.shape[0],self.batch_size):                       
                    self.w_updt = np.zeros((self.input, weight_size))
                    self.bias_h_updt = np.zeros((1,weight_size))
                    self.bias_v_updt = np.zeros((1,self.input)) 
                    ...
                    t0 = time.time()
                    error += np.mean((self.input_dropped-self.input_original)**2)
